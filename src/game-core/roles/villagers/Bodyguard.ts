...
export class Bodyguard implements IRole {
    ...
    readonly description = 'Mỗi đêm, bạn chọn một người để bảo vệ. Nếu người đó bị Sói cắn, họ sẽ không chết.';
    ...
    getUiContext(gameState: GameState, self: Player): any {
        return {
            availableTargets: gameState.getLivingPlayers().filter(p => p.id !== self.id)
