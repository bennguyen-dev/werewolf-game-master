...
export class Cupid implements IRole {
    ...
    readonly description = 'Đêm đầu tiên, bạn chọn hai người để ghép đôi. Nếu một trong hai chết, người còn lại cũng sẽ chết theo.';

    handleGameEvent(event: GameEvent, gameState: GameState, self: Player): IAction[] | null {
        // Cupid only acts on the first night.
        if (event.type === 'PHASE_CHANGED' && event.payload.newPhase === 'NIGHT' && event.payload.day === 1) {
            // Ready to act
        }
        return null;
    }
    ...
    getUiContext(gameState: GameState, self: Player): any {
        ...
        return {
            canAct: true,
            availableTargets: gameState.getLivingPlayers().map(p => ({ id: p.id, name: p.name }))
        };
    }
}
